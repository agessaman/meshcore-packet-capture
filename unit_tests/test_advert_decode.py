...
def test_short_advert_payload_is_rejected(capture: PacketCapture) -> None:
...
def test_valid_advert_payload_decodes_with_required_fields(capture: PacketCapture) -> None:
    raw_hex = _build_packet_hex(_make_valid_advert_payload())
    decoded = capture.decode_and_publish_message(raw_hex)
    assert decoded is not None
    assert decoded["payload_type"] == PayloadType.ADVERT.name
    assert decoded["advert_parse_ok"]
    assert "public_key" in decoded
    assert "advert_time" in decoded
    assert "signature" in decoded


def test_packed_two_byte_hops_slice_payload_correctly(capture: PacketCapture) -> None:
    ...
    decoded = capture.decode_and_publish_message(raw_hex)
    assert decoded is not None
    assert decoded["path_hash_bytes"] == 2
    assert decoded["path_byte_len"] == 4
    assert decoded["advert_parse_ok"]


def test_issue24_sample_does_not_raise_keyerror(capture: PacketCapture) -> None:
    sample_hex = (
    ...
    decoded = capture.decode_and_publish_message(sample_hex)
    if decoded is not None and decoded.get("payload_type") == PayloadType.ADVERT.name:
        assert "public_key" in decoded


def test_reserved_mode_falls_back_to_legacy_length(capture: PacketCapture) -> None:
    path_byte_len, path_hash_bytes = capture._decode_packed_path_length(0xC2)
    assert path_byte_len == 0xC2
    assert path_hash_bytes == 1
